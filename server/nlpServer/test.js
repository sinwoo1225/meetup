...
async function executor(req){
    if(is_initial==false){ //중복 실행 시 오류 방지
    ...
    for(const sent of parsed){
        let index = 0;
        for(const dep of sent.dependencies){ //각 문장별 형태소 추출
            let len = dep.toString().length;
            console.log(dep.toString());
            while(true){ //NNG 형태소(단어) 추출하고 갯수 세는 알고리즘
                if(!dep.toString().substring(index, len).includes('/NNG') == true) break;
                let temp = dep.toString().substring(index, len).match(regular)[0].substring(0, dep.toString().match(regular)[0].indexOf('/')); //temp 인덱스부터 /NNG전까지 문자열
                if(temp.indexOf(' ')+temp.indexOf('+')==-2) console.log("string length error!"); //이하 /NNG와 결합된 단어 추출 temp에 덮어쓰기
                else if(temp.indexOf(' ')==-1) temp = temp.substring(temp.lastIndexOf('+')+1, temp.length);
                else if(temp.indexOf('+')==-1) temp = temp.substring(temp.lastIndexOf(' ')+1, temp.length);
                else temp = (temp.lastIndexOf(' ')<temp.lastIndexOf('+') ? temp.substring(temp.lastIndexOf(' ')+1, temp.length) : temp.substring(temp.lastIndexOf('+')+1, temp.length));
                //console.log(temp);
